	...
	if(isset($_POST['customerDetailsCustomerID'])) {
		...
		if(isset($customerDetailsCustomerFullName) && isset($customerDetailsCustomerMobile) && isset($customerDetailsCustomerAddress)) {
			...
			if(empty($customerDetailsCustomerID)){
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Harap masukkan ID Pelanggan untuk update data. Anda dapat menemukan ID Pelanggan pada menu Pencarian</div>';
				exit();
			}
			...
			if($customerIDSelectStatement->rowCount() > 0) {
				...
				echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Pelanggan berhasil diupdate.</div>';
				exit();
			} else {
				// CustomerID is not in DB. Therefore, stop the update and quit
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Tidak dapat update pelanggan. Pelanggan tidak terdaftar di DB.</div>';
				exit();
			}
