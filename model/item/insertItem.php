	...
	if(isset($_POST['itemDetailsItemNumber'])){
		...
		if(!empty($itemNumber) && !empty($itemName) && isset($quantity) && isset($unitPrice)){
			...
			if($stockStatement->rowCount() > 0){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Item sudah terdaftar di DB. Silakan klik tombol <strong>Update</strong> untuk memperbarui data. Atau gunakan Nomor Item yang berbeda.</div>';
				exit();
			} else {
				...
				echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Item berhasil ditambahkan.</div>';
				exit();
			}
