	...
	if(isset($_POST['purchaseDetailsPurchaseID'])){
		...
		if(isset($purchaseDetailsItemNumber) && isset($purchaseDetailsPurchaseDate) && isset($purchaseDetailsQuantity) && isset($purchaseDetailsUnitPrice)){
			...
			if(filter_var($purchaseDetailsUnitPrice, FILTER_VALIDATE_FLOAT) === 0.0 || filter_var($purchaseDetailsUnitPrice, FILTER_VALIDATE_FLOAT)){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Harap masukkan nilai yang valid untuk harga satuan.</div>';
				exit();
			}
			
			// Check if purchaseID is empty
			if($purchaseDetailsPurchaseID == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Harap masukkan ID pembelian.</div>';
				exit();
			}
			...
			if($originalPurchaseQuantityStatement->rowCount() > 0){
				...
				if($originalOrderItemNumber !== $purchaseDetailsItemNumber) {
					...
					if($newItemCurrentStockStatement->rowCount() < 1){
						...
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Item tidak terdaftar pada DB.</div>';
						exit();
					}
					...
					echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Data pembelian berhasil diupdate.</div>';
					exit();
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Data pembelian tidak terdaftar di DB.</div>';
				exit();
