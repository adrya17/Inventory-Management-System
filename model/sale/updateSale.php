	...
	if(isset($_POST['saleDetailsSaleID'])){
		...
		if(isset($saleDetailsItemNumber) && isset($saleDetailsSaleDate) && isset($saleDetailsQuantity) && isset($saleDetailsUnitPrice) && isset($saleDetailsCustomerID)){
			...
			if($saleDetailsSaleID == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Harap masukkan ID penjualan.</div>';
				exit();
			}
			...
			if($saleDetailsQuantity == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Harap masukkan kuantitas.</div>';
				exit();
			}
			...
			if($customerIDStatement->rowCount() < 1){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>ID customer tidak terdaftar di DB. Harap masukkan ID customer yang valid.</div>';
				exit();
			} else {
				$row = $customerIDStatement->fetch(PDO::FETCH_ASSOC);
				$customerID = $row['customerID'];
			...
			if($originalSaleQuantityStatement->rowCount() > 0){
				...
				if($originalOrderItemNumber !== $saleDetailsItemNumber) {
					...
					if($newItemCurrentStockStatement->rowCount() < 1){
						...
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Kode item tidak terdaftar di DB.</div>';
						exit();
					}
					...
					echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Data penjualan berhasil diupdate.</div>';
					exit();
					...
					if($stockStatement->rowCount() > 0){
						...
						// sale details for it 
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Item tidak terdaftar pada DB, harap daftarkan item terlebih dahulu pada menu <strong>Item</strong></div>';
						exit();
					}	
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Data penjualan tidak terdaftar di DB.</div>';
				exit();
