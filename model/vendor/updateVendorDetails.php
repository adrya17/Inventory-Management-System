	...
	if(isset($_POST['vendorDetailsVendorID'])) {
		...
		if(!empty($vendorDetailsVendorID)){
			...
			if(!empty($vendorDetailsVendorFullName) && !empty($vendorDetailsVendorMobile) && !empty($vendorDetailsVendorAddress)) {
				...
				if(empty($vendorDetailsVendorID)){
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Harap masukkan ID Supplier untuk update data. Anda dapat menemukan ID Supplier pada menu Pencarian</div>';
					exit();
				}
				...
				if(!empty($vendorDetailsVendorEmail)) {
					if (filter_var($vendorDetailsVendorEmail, FILTER_VALIDATE_EMAIL) === false) {
						// Email is not valid
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Harap masukkan alamat email yang valid.</div>';
						exit();
					}
				}

				// Validate address
				if($vendorDetailsVendorAddress == ''){
					// Address 1 is empty
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Harap isi bidang alamat.</div>';
					exit();
				}
				...
				if($vendorIDSelectStatement->rowCount() > 0) {
					...
					echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Supplier berhasil diupdate.</div>';
					exit();
				} else {
					// vendorID is not in DB. Therefore, stop the update and quit
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Tidak dapat update supplier. Supplier tidak terdaftar di DB.</div>';
					exit();
				}
